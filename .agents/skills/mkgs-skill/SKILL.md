...
## 3. Never change the database schema directly — always migrate

CRITICAL: Structural database changes go through the project's migration tool — never by editing schema files or the live DB by hand.

- Generate a migration, review it, run it (Alembic, Prisma Migrate, Rails, Django, Flyway, Diesel, golang-migrate, Sequelize, etc.).
- Don't hand-edit the generated schema/snapshot file (`schema.prisma`'s output, `schema.rb`, a committed `schema.sql` dump), and don't `ALTER` the live database directly to make a lasting change.
- Migrations are ordered, reversible, and reproducible across every environment; a direct edit silently drifts production from the code and can't be replayed.
- Match the project's existing migration tool. If none exists and the change is real, set one up rather than editing the schema by hand — say so when you do.

## 4. Commit fast and often
...
## 5. Run on every operating system
...
- Avoid shelling out to platform-specific binaries (`cp`, `rm`, `which`, `open`, `xdg-open`) when a portable module call does the same job — see §6.
...
## 6. Don't shell out where a module does the job
...
- Module calls are cross-platform (§5), return typed results, surface real errors instead of parsing stdout, and dodge shell-injection and quoting bugs.
...
## 7. Prefer Rust when the language is open
...
- New standalone tool, script, or service with a free choice → reach for Rust: one static binary, cross-platform out of the box (§5), strong types, no runtime to ship.
...
## 12. Keep fallbacks only when necessary and beneficial

CRITICAL: A fallback that hides a failure is worse than the failure. Default to letting it fail loudly.

- Keep a fallback when degraded operation is genuinely better than stopping — a cache miss refetching, a retry on a flaky network, an optional feature the app can run without.
- Drop it when it papers over a bug: `except: pass`, a default value substituted for a failed lookup, a stub returned when a dependency is missing, a second code path kept "just in case".
- Never let a fallback silently change behaviour. If one fires, log it with the real error and say what was substituted.
- Two paths mean two paths to test and two to debug at 3am. If the fallback path can't be justified in one sentence, delete it and let the error surface.
